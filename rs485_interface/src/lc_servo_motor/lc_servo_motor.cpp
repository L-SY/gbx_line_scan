...
{
  // Convert RPM to Pr/s
  // According to manual example: 20000 Pr/s = 120 RPM
  // Direct conversion: Pr/s = RPM * (20000 / 120) = RPM * 166.666...
  // This is equivalent to: Pr/s = RPM * (encoder_resolution / 60)
  // where encoder_resolution = 10000 pulses/rev
  // 
  // Use the direct conversion from manual to avoid any confusion
  const double RPM_TO_PRS = 20000.0 / 120.0;  // = 166.666...
  double speed_prs = speed_rpm * RPM_TO_PRS;
  uint32_t speed_prs_int = static_cast<uint32_t>(speed_prs + 0.5);  // Round to nearest
  
  // Debug: Log the conversion
  // Note: This will be logged by the GUI, but we can add it here too if needed
